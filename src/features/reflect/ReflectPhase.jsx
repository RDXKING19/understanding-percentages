...
const TAKEAWAYS = [
  { icon: '💯', text: 'Percent means "per hundred" — it tells us how many out of every 100.' },
  { icon: '🔁', text: 'A percent, a fraction, and a decimal can all represent the exact same amount.' },
  { icon: '🧮', text: 'To find a percent of a number, multiply: percent ÷ 100 × the number.' },
  { icon: '📊', text: 'A 10x10 grid is a handy picture for percent — each square is exactly 1%.' },
];

const CONFIDENCE_OPTIONS = [
  { emoji: '🌟', text: "I'm great at understanding percentages!" },
  { emoji: '👍', text: 'I can work out most percentages!' },
    ...
    return (
      <div className="reflect-screen" style={{ justifyContent: 'center' }}>
        <div className="reflect-card glass-card" style={{ width: '100%', maxWidth: 520, textAlign: 'center' }}>
          ...
          <p style={{ fontWeight: 700, marginBottom: 16 }}>How confident do you feel about understanding percentages?</p>
      ...
      <div style={{ width: '100%', maxWidth: 560, display: 'flex', flexDirection: 'column', gap: 14, paddingBottom: 16 }}>
        ...
        <div className="reflect-card glass-card" style={{ textAlign: 'center' }}>
          ...
          <p style={{ color: 'var(--color-text-muted)', fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: '0.9rem', marginTop: 6 }}>
            {pct >= 80 ? "Outstanding! You're a Percent Pro! 🏆" : pct >= 60 ? "Well done! Keep practising! 💪" : "Review the key facts below 📚"}
          </p>
        </div>
        ...
        <div className="conversion-fact-box">
          <span className="mass-value">Percent = Per Hundred!</span>
          <div style={{ fontSize: '0.85rem', marginTop: 4, color: 'rgba(255,255,255,0.65)' }}>
            25% = 25/100 = 0.25 — three ways to write the exact same amount
          </div>
        </div>
        ...
        <div className="reflect-badge glass-card">
          ...
          <div>
            ...
            <div className="reflect-badge-sub">Understanding Percentages</div>
          </div>
        </div>
